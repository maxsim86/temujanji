...
class CiptaTemujanjiWizardView(SessionWizardView):
    ...
    def get_context_data(self, form, **kwargs):
        ...
        if self.steps.current=='Masa':
            context['dapatkan_masa_available'] = dapatkan_masa_available(self.get_cleaned_data_for_step('Tarikh')['tarikh'])
            progress_width ='30'
...
def dapatkan_masa_available(date: datetime.date) -> List[Dict[datetime.time, bool]]:
    ...
    while True:
        ...
        next_time = add_delta(next_time, datetime.timedelta(
            minutes=int(tempahan_settings.period_of_each_booking)))
